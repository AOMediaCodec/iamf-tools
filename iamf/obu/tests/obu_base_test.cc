...
class ImaginaryObuNonIntegerBytes : public ObuBase {
 public:
  ImaginaryObuNonIntegerBytes() : ObuBase(kObuIaReserved25) {}
...
TEST(ObuBaseTest, InvalidWhenValidatePayloadDerivedDoesNotReadIntegerBytes) {
  ...
  std::vector<uint8_t> source_data = {kObuIaReserved25 << 3, 1, 0x80};
...
class OneByteObu : public ObuBase {
 public:
  OneByteObu() : ObuBase(kObuIaReserved25) {}
  explicit OneByteObu(const ObuHeader& header)
      : ObuBase(header, kObuIaReserved25) {}
...
TEST(ObuBaseTest, ReadWithConsistentSize) {
  std::vector<uint8_t> source_data = {kObuIaReserved25 << 3, 1, 255};
  auto rb = MemoryBasedReadBitBuffer::CreateFromSpan(
      absl::MakeConstSpan(source_data));
...
TEST(ObuBaseTest, ReadDoesNotOverflowWhenBufferIsLarge) {
  ...
  const std::vector<uint8_t> kObu = {kObuIaReserved25 << 3, 1, 255};
