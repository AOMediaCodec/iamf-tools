...
TEST(LibFlacCallbackData, ConstructorSetsNumSamplesPerChannel) {
  ...
  LibFlacCallbackData callback_data(kNumSamplesPerFrame, kNumChannels,
                                    decoded_frame);
...
TEST(LibFlacWriteCallback, SucceedsFor32BitSamples) {
  ...
  std::vector<std::vector<InternalSampleType>> decoded_frame;
  LibFlacCallbackData callback_data(kThreeSamplesPerFrame, kNumChannels,
                                    decoded_frame);
...
TEST(LibFlacWriteCallback, SucceedsFor16BitSamples) {
  ...
  std::vector<std::vector<InternalSampleType>> decoded_frame;
  LibFlacCallbackData callback_data(kTwoSamplesPerFrame, kNumChannels,
                                    decoded_frame);
...
TEST(LibFlacWriteCallback, FillsExtraSamplesWithZeros) {
  ...
  // channel.
  std::vector<std::vector<InternalSampleType>> decoded_frame;
  LibFlacCallbackData callback_data(kFourSamplesPerFrame, kNumChannels,
                                    decoded_frame);
