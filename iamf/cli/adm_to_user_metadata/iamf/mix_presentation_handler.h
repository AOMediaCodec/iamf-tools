...
class MixPresentationHandler {
 public:
  // Used to generate localized element annotations when there is no
  // `audioObjectLabel` or `audioObjectName`.
  // Used when the source ADM carried neither an `audioProgrammeLabel` nor an
  // `audioProgrammeName` for this mix presentation.
  static constexpr absl::string_view kDefaultLocalizedPresentationAnnotations =
      "test_mix_pres";
