...
4. **Opus Ambisonics Recommended Practices** *(Optional)*:
   * Invokes `opus_hoa` to verify that Opus Ambisonics Audio Elements adhere to recommended coding mode and demixing matrix practices.
...
* `--opus_hoa_cmd`: (Optional) Path to the Opus Ambisonics verifier binary. If unspecified or `None`, this verification stage is bypassed. *(Note: When compiling standalone via Bazel, the binary is located at `bazel-bin/validation/opus_hoa/opus_hoa_main`).*
